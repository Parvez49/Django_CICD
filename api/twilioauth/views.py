...
class VerifyOTPView(View, TwilioBaseView):
    def post(self, request, *args, **kwargs):
        ...
        if not phone_number or not otp_code:
            return JsonResponse(
                {"error": "Phone number and OTP code are required"}, status=400
            )
...
class SendEmailOTPView(View, TwilioBaseView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            ...
            otp_record = OTP(email=email, otp=otp, expires_at=valid_time)
            otp_record.save()
        ...
        if request.method == "POST":
            ...
            try:
                ...
                if otp_record.is_expired():
                    return JsonResponse(
                        {"error": "One time password has expired"}, status=400
                    )
                ...
                if otp_record.otp == otp:
                    ...
                    return JsonResponse(
                        {"message": "OTP validation Success", "user_data": user_data},
                        status=200,
                    )
