...
def register_user(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            login(request, user)
            messages.success(
                request, ("You have successfullt register. Welcome to Lovetamin")
            )
            return redirect("home")
...
def unfollow(request, pk):
    if request.user.is_authenticated:
        ...
        # Return message
        messages.success(
            request, (f"You have unfollowed this user {profile.user.username}")
        )
        return redirect(request.META.get("HTTP_REFERER"))
...
def follow(request, pk):
    if request.user.is_authenticated:
        ...
        if profile not in request.user.profile.follows.all():
            ...
            # Return message
            messages.success(
                request, (f"You have followed this user {profile.user.username}")
            )
            return redirect(request.META.get("HTTP_REFERER"))
        else:
            messages.success(
                request,
                (f"You have already followed this user {profile.user.username}"),
            )
            return redirect(request.META.get("HTTP_REFERER"))
...
def search_post(request):
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            return render(
                request, "search_post.html", {"search": search, "posts": posts}
            )
        else:
    ...
    if request.user.is_authenticated:
        if request.method == "POST":
            ...
            return render(
                request,
                "search_user.html",
                {"search_user": search_user, "users": users},
            )
        else:
...
def update_preferance(request):
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            create_preference = UserPreference.objects.create(
